...
import ru.bpc.cm.encashments.AtmEncashmentSummsForDateController;
import ru.bpc.cm.items.encashments.EncashmentCassItem;
import ru.bpc.cm.utils.Pair;

/**
 * Интерфейс-маппер для класса {@link AtmEncashmentSummsForDateController}.
 * 
 ...
 *
 */
public interface AtmEncashmentSummsForDateMapper extends IMapper {

	@Result(column = "ENC_COUNT", javaType = Integer.class)
	@Select("SELECT count(distinct ep.ENC_PLAN_ID) as ENC_COUNT FROM T_CM_TEMP_ENC_PLAN ep "
			+ "join T_CM_TEMP_ENC_PLAN_DENOM epd on (ep.ENC_PLAN_ID = epd.ENC_PLAN_ID) WHERE "
	...
	@Select("SELECT epd.DENOM_VALUE, sum(epd.DENOM_COUNT) as DENOM_COUNT, epd.DENOM_CURR , ci.CODE_A3 "
			+ "FROM T_CM_TEMP_ENC_PLAN ep join T_CM_TEMP_ENC_PLAN_DENOM epd on(ep.ENC_PLAN_ID = epd.ENC_PLAN_ID) "
